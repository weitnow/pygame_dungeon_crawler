...
class Weapon():
    ...
    def update(self, player):
        ...
        y_dist = -(pos[1] - self.rect.centery)  # is negative, because pygame y coordinates increase down the screen
...
class Arrow(pygame.sprite.Sprite):
    def __init__(self, image, x, y, angle):
        ...
        self.dy = -(math.sin(math.radians(
            self.angle)) * constants.ARROW_SPEED)  # - because pygame y coordinates increases down the screen
    ...
    def update(self, enemy_list):
        ...
        if self.rect.right < 0 or self.rect.left > constants.SCREEN_WIDTH \
                or self.rect.bottom < 0 or self.rect.top > constants.SCREEN_HEIGHT:
            self.kill()
